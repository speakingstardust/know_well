class JigsController < ApplicationController
  ...
  def destroy
    authorize @jig
    if @jig.jig_work_order_line_items.any? 
      redirect_to jigs_url, notice: "Jig is currently used in a Jig Work Order and cannot be deleted at this time. Please remove the jig from all relevant Jig Work Orders before trying again."
    else
      @jig.destroy
      redirect_to jigs_url, notice: 'Jig was successfully destroyed.'
    end
  end
  ...
  private
    ...
    def jig_params
      params.require(:jig).permit(:name, :cleaning_charge, :repair_charge, :jig_photo, :customer_id, :aka)
    end
end
